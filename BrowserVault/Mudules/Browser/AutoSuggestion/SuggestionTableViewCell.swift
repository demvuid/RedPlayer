...
class SuggestionTableViewCell: UITableViewCell {
    ...
    func updateFor(query: String, suggestion: Suggestion) {
        ...
        if URL.isWebUrl(text: text) {
            typeImage.image = Asset.Tabbar.iconBrowser.image.withRenderingMode(.alwaysTemplate)
        } else {
            
            typeImage.image = Asset.General.icSearchWeb.image.withRenderingMode(.alwaysTemplate)
        }
    ...
    private func styleText(query: String, text: String) {
        ...
        let attributes = [
            NSAttributedString.Key.font: AppBranding.boldFont,
            NSAttributedString.Key.foregroundColor: ColorName.titleColor
        ]
