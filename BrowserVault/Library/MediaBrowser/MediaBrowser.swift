...
open class MediaBrowser: UIViewController, UIScrollViewDelegate, UIActionSheetDelegate, AVPlayerViewControllerDelegate {
    ...
    /**
     ...
     - Parameter media: Media array
     */
    public convenience init(folder: FolderModel) {
        self.init()
        self.folder = folder
        fixedMediasArray = folder.medias.map({$0})
    }
    ...
    private func initialisation() {
        ...
        if #available(iOS 11.0, *) {
            pagingScrollView.contentInsetAdjustmentBehavior = .automatic
        } else {
...
//        }
        ...
        if displayActionButton {
            actionButton = UIBarButtonItem(
                image: UIImage.imageForResourcePath(name: "ic_file_check", inBundle: Bundle(for: MediaBrowser.self)),
                style: .plain,
                target: self,
...
//        releaseAllUnderlyingPhotos(preserveCurrent: true)
    ...
    func mediaAtIndex(index: Int) -> Media? {
        ...
        if index < mediaArray.count && index >= 0 {
            if mediaArray[index] == nil {
                let blockUpdateMediaArray = {[unowned self] in
                    if nil == media && self.fixedMediasArray != nil && index < self.fixedMediasArray!.count {
                        media = self.fixedMediasArray![index]
                    }
                    media?.placeholderImage = self.placeholderImage?.image
                    
                    if media != nil {
                        self.mediaArray[index] = media
                    }
                }
                if let d = delegate {
                    media = d.media(for: self, at: index)
                    blockUpdateMediaArray()
                } else {
                    blockUpdateMediaArray()
                }
            } else {
    ...
    func thumbPhotoAtIndex(index: Int) -> Media? {
        ...
        if index < thumbMedias.count {
            if nil == thumbMedias[index] {
                let blockUpdateThumbnailArray = {[unowned self] in
                    if nil == media && self.fixedMediasArray != nil && index < self.fixedMediasArray!.count {
                        media = self.fixedMediasArray![index]
                    }
                    media?.placeholderImage = self.placeholderImage?.image
                    ...
                    if media != nil {
                        self.thumbMedias[index] = media
                    }
                }
                if let d = delegate {
                    media = d.thumbnail(for: self, at: index)
                    blockUpdateThumbnailArray()
                } else {
                    blockUpdateThumbnailArray()
                }
            } else {
    ...
    func captionViewForPhotoAtIndex(index: Int) -> MediaCaptionView? {
        ...
        } else if let p = mediaAtIndex(index: index) {
            if p.caption.count > 0 {
                captionView = MediaCaptionView(media: p)
            }
        }
    ...
    func updateNavigationBar() {
        if displaySelectionButtons {
            if selections.filter({$0 == true}).count > 0 {
                self.navigationItem.setRightBarButtonItems([addItem, deleteItem], animated: true)
            } else {
                self.navigationItem.setRightBarButtonItems([addItem], animated: true)
            }
            
        } else {
            if gridController != nil && numberOfMedias > 0 {
                self.navigationItem.setRightBarButtonItems([addItem, actionButton].compactMap({$0}), animated: true)
            } else {
                self.navigationItem.setRightBarButtonItems([addItem], animated: true)
            }
        }
    ...
    func startPreCaching() {
        ...
        } else if self.fixedMediasArray != nil && currentPageIndex < self.fixedMediasArray!.count {
            let media = self.fixedMediasArray![currentPageIndex]
            media.loadUnderlyingImageAndNotify()
        } else {
...
//            }
    ...
    func showGrid(animated: Bool) {
        ...
        if let gc = gridController {
            ...
            gc.view.frame = CGRect(x: 0, y: self.navigationController?.navigationBar.frame.maxY ?? 0, width: bounds.width, height: bounds.height)
            gc.view.alpha = 0.0
    ...
    @objc func emptyFiles() {
        if displaySelectionButtons, selections.filter({$0 == true}).count > 0 {
            let mediaSelections = self.selections.enumerated().filter({$1 == true}).compactMap({self.mediaAtIndex(index: $0.offset)})
            if mediaSelections.count > 0 {
                let message = mediaSelections.count == 1 ? L10n.Folder.Confirm.Delete.file : L10n.Folder.Confirm.Delete.files(mediaSelections.count)
                self.showAlertWith(title: L10n.Generic.warning, message: message, cancelTitle: L10n.Generic.Button.Title.cancel, destrucionTitle: L10n.Generic.delete) {[weak self] (_) in
